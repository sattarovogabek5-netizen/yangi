...
<head>
  ...
  <style>
    ...
    .auto-send {margin-top:12px; padding:10px; background:#e8f5e8; border-radius:8px; border:1px solid #c8e6c9;}
    .auto-send h4 {margin:0 0 6px 0;font-size:14px; text-align:center}
    .performance-options {margin-top:12px; padding:10px; background:#f3e5f5; border-radius:8px; border:1px solid #e1bee7;}
    .performance-options h4 {margin:0 0 6px 0;font-size:14px; text-align:center}
  </style>
</head>
<body>
  ...
  <div class="performance-options">
    <h4>Ishlash Sozlamalari</h4>
    <div class="controls">
      <label><input type="checkbox" id="highPerformance" checked> Yuqori tezlik rejimi</label>
      <label><input type="checkbox" id="adaptiveScanning" checked> Adaptiv skanerlash</label>
    </div>
  </div>
  ...
  <div class="auto-send">
    <h4>Avtomatik Botga Yuborish</h4>
    <div class="controls">
      <label><input type="checkbox" id="autoSendToBot"> QR kod topilganda avtomatik botga yuborish</label>
    </div>
    <div class="hint" style="margin-top:8px">
      <input type="text" id="botUrl" placeholder="Bot URL manzili (ixtiyoriy)" style="width:100%;padding:6px;border-radius:4px;border:1px solid #ddd">
    </div>
  </div>
  ...
  <script>
    ...
    const autoSendToBot = document.getElementById('autoSendToBot');
    const botUrl = document.getElementById('botUrl');
    const highPerformance = document.getElementById('highPerformance');
    const adaptiveScanning = document.getElementById('adaptiveScanning');
    ...
    let lastScanTime = 0;
    let scanInterval = 0;
    let adaptiveScanArea = null;
    let scanAreaSize = 1.0;
    ...
    async function scanLoop() {
      ...
      scanInterval = highPerformance.checked ? 100 : 200;
      ...
      if (video.readyState === video.HAVE_ENOUGH_DATA) {
        ...
        // Adaptive scanning - focus on center area if enabled
        let scanArea = null;
        if (adaptiveScanning.checked && adaptiveScanArea) {
          scanArea = adaptiveScanArea;
        } else {
          // Calculate adaptive scan area based on performance
          scanAreaSize = highPerformance.checked ? 0.7 : 1.0;
          const centerX = vw / 2;
          const centerY = vh / 2;
          const areaWidth = vw * scanAreaSize;
          const areaHeight = vh * scanAreaSize;
          
          scanArea = {
            x: centerX - areaWidth/2,
            y: centerY - areaHeight/2,
            width: areaWidth,
            height: areaHeight
          };
          
          adaptiveScanArea = scanArea;
        }
        ...
        if (code) {
          ...
          // Auto send to bot if enabled
          if (autoSendToBot.checked) {
            await sendToBot(code.data);
            // Don't show result panel if auto-sending
          ...
          // Draw adaptive scan area if enabled
          if (adaptiveScanning.checked && adaptiveScanArea) {
            octx.strokeStyle = 'rgba(0,255,255,0.2)';
            octx.lineWidth = 1;
            octx.strokeRect(
              adaptiveScanArea.x * (overlayCanvas.width / vw),
              adaptiveScanArea.y * (overlayCanvas.height / vh),
              adaptiveScanArea.width * (overlayCanvas.width / vw),
              adaptiveScanArea.height * (overlayCanvas.height / vh)
            );
          }
    ...
    async function sendToBot(text) {
      ...
      // Use provided bot URL or default
      const botEndpoint = botUrl.value.trim() || 'https://api.example.com/bot';
      
      try {
        statusInfo.textContent = 'Botga yuborilmoqda...';
        statusInfo.style.background = '#fff8e1';
        statusInfo.style.border = '1px solid #ffecb3';
        ...
        // if (response.ok) {
        //   alert(`QR kod ma'lumoti botga muvaffaqiyatli yuborildi!`);
        // } else {
        ...
        statusInfo.textContent = 'QR kod topildi va botga yuborildi!';
        statusInfo.style.background = '#e8f5e8';
        statusInfo.style.border = '1px solid #c8e6c9';
        
      } catch (error) {
        console.error('Botga yuborishda xatolik:', error);
        alert('Botga yuborishda xatolik yuz berdi: ' + error.message);
        
        statusInfo.textContent = 'Botga yuborishda xatolik';
        statusInfo.style.background = '#ffebee';
        statusInfo.style.border = '1px solid #ffcdd2';
      }
    }
    ...
    sendToBotBtn.addEventListener('click', () => {
      ...
      sendToBot(txt).then(() => {
        // Close the result panel after sending
        resultPanel.style.display = 'none';
        backdrop.style.display = 'none';
        scanning = true;
        scanLoop();
